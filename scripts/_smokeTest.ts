...
for (const id of ids) {
  ...
  const topComments = (
    await Promise.all(post.kids.slice(0, 3).map(fetchCommentText))
  ).filter(Boolean) as string[];
  const bottomIds = fetchBottomCommentIds(post.kids, 3);
  const bottomComments = (
    await Promise.all(bottomIds.map(fetchCommentText))
  ).filter(Boolean) as string[];
  const articleText = post.url ? await fetchArticleText(post.url) : null;

  const { relevance } = await LlmClassifier.classify({
    ...post,
    topComments,
    bottomComments,
    articleText,
